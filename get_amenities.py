...
# Configurar os parâmetros de consulta para amenities
amenities_tags = {'amenity': True}
print(f'As amenities: {amenities_tags}')

# Baixar os dados das amenities
logger.info("Baixando dados das amenities...")
try:
    amenities = ox.geometries_from_place(city, amenities_tags)
    logger.info(f"Número de amenities encontradas: {len(amenities)}")
except Exception as e:
    logger.error(f"Erro ao baixar dados das amenities: {e}")
    amenities = None
...
if amenities is not None and admin_boundaries is not None:
    # Preparar os dados das amenities
    data = []
    for idx, row in amenities.iterrows():
        ...
        data.append([name, amenity_type, address, latitude, longitude, freguesia, opening_hours])
    ...
    df.to_csv('./data/porto_amenities_with_freguesia.csv', index=False)
